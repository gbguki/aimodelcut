...
export const generateFashionImage = async (
  ...
  const hasReferenceImage = !!config.referenceImage;
  ...
  // 참고 이미지가 있으면 포즈/구도 참고 지시 추가
  if (hasReferenceImage) {
    editInstruction += `
    [REFERENCE IMAGE PROVIDED]: A reference image for pose/composition is provided.
    - Use the POSE and COMPOSITION from the reference image
    - KEEP the model's face and identity from the previous generated image (if exists)
    - Apply the body position, angle, and framing from the reference
    - Do NOT copy the reference model's face or identity - only the pose/composition
    `;
  }
  ...
  // Reference Image (포즈/구도 참고 이미지)
  if (config.referenceImage) {
    try {
      const refData = await ensureBase64(config.referenceImage);
      parts.push({
        inlineData: {
          data: refData.base64,
